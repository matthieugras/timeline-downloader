...
func run(cmd *cobra.Command, args []string) error {
	...
	// Generate device jobs (with chunking if enabled)
	var jobs []*worker.Job
	jobID := 0
	...
	// Generate identity jobs (with chunking if enabled)
	for _, identityStr := range cfg.Identities {
		identity := api.NewIdentityInput(identityStr)
		identityJobs := worker.SplitIdentityIntoChunks(identity, cfg.FromDate, cfg.ToDate, cfg.TimeChunkDays, jobID)
	...
	for _, job := range jobs {
		if job.ChunkInfo != nil {
			chunkedEntities[job.ChunkInfo.DeviceKey] = true
		}
	}
